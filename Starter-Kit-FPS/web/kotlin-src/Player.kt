...
import net.multigesture.kanama.api.findChildren
...
/**
 * Web port of the FPS Player. Adaptations from the desktop port: camera-relative movement uses the
 * pure-Kotlin rotation Basis (scale-1 body); the weapon-swap tween hangs off the node's own
 * create_tween; impact decals parent to the scene root via getParent (SceneTree.get_root has no Web
 * family) and orient with the baked-default look (the decal sprite is double-faced); ray hits
 * damage the Enemy script directly through the tracked collider handle.
 */
...
class Player(godotObject: GodotHandle) :
  ...
  private val containerOffset = Vector3(1.2, -1.1, -2.75)
  ...
  @Signal fun healthUpdated(health: Long) = Unit
  ...
  fun ready() {
    ...
    muzzle =
      self.requireAs(
        "Head/Camera/SubViewportContainer/SubViewport/CameraItem/Muzzle",
        ::AnimatedSprite3D,
      )
    container =
      self.requireAs("Head/Camera/SubViewportContainer/SubViewport/CameraItem/Container", ::Node3D)
  ...
  fun process(delta: Double) {
    ...
    val movedForward = self.basis * movementVelocity
    ...
    if (
      self.isOnFloor() && (Mathf.abs(self.velocity.x) > 1.0 || Mathf.abs(self.velocity.z) > 1.0)
    ) {
  ...
  private fun handleControls(delta: Double) {
    ...
    movementVelocity = Vector3(input.x, 0.0, input.y).normalized() * movementSpeed.toDouble()
  ...
  private fun handleRotation(
    ...
    if (isController) {
      ...
      rotationTarget =
        rotationTarget.withX(GD.clampf(rotationTarget.x, GD.degToRad(-90.0), GD.degToRad(90.0)))
      camera.rotation =
        camera.rotation.withX(GD.lerpAngle(camera.rotation.x, rotationTarget.x, delta * 25.0))
      self.rotation =
        self.rotation.withY(GD.lerpAngle(self.rotation.y, rotationTarget.y, delta * 25.0))
    } else {
  ...
  private fun actionShoot() {
    ...
    repeat(weapon.shotCount.toInt()) {
      ...
      raycast.getCollider()?.kotlinScriptInstance<Enemy>()?.damage(weapon.damage)

      val impact = impactScene.instantiate()?.let { AnimatedSprite3D(it.handle) } ?: return@repeat
      impact.play("shot")
      Node(checkNotNull(self.getParent()) { "Player requires a scene parent" }.handle)
        .addChild(impact)
      impact.position = raycast.getCollisionPoint() + (raycast.getCollisionNormal() / 10.0)
      impact.lookAtFromPosition(impact.position, camera.globalPosition)
    }

    val knockback = randomVec2(weapon.minKnockback, weapon.maxKnockback)
    container.position = container.position.withZ(container.position.z + 0.25)
    ...
    rotationTarget = rotationTarget + Vector3(knockback.x, knockback.y, 0.0)
    movementVelocity += Vector3(0.0, 0.0, weapon.knockback.toDouble())
  }
  ...
  fun changeWeapon() {
    ...
    val weaponModelScene =
      weapon.model ?: error("Weapon at index $weaponIndex requires a model PackedScene")
    val weaponCrosshair =
      weapon.crosshair ?: error("Weapon at index $weaponIndex requires a crosshair Texture2D")
    ...
    val weaponModel =
      weaponModelScene.instantiate()
        ?: error("Weapon at index $weaponIndex failed to instantiate its model")
    container.addChild(Node(weaponModel.handle))

    val weaponNode = Node3D(weaponModel.handle)
    weaponNode.position = weapon.position
    weaponNode.rotationDegrees = weapon.rotation

    for (child in Node(weaponModel.handle).findChildren("*", "MeshInstance3D")) {
      MeshInstance3D(child.handle).setLayerMask(2)
    }

    raycast.targetPosition = Vector3(0.0, 0.0, -1.0) * weapon.maxDistance.toDouble()
  ...
  private fun randomVec2(min: Vector2, max: Vector2): Vector2 {
    val sign = if (GD.randi().mod(2L) == 0L) -1.0 else 1.0
    return Vector2(GD.randfRange(min.x, max.x), GD.randfRange(min.y, max.y) * sign)
  }
