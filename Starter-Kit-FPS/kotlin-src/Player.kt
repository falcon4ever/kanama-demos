 ...
 * Web family: the weapon-swap tween hangs off the node's own create_tween (SceneTree.create_tween
 * is desktop-only; Node.create_tween binds to the same tree tween), and impact decals parent to
 * the Player's scene parent via getParent (SceneTree.get_root is desktop-only; the parent has an
 * identity transform here, but decals now free with the scene on reload). PackedScene.instantiate
 * returns the base wrapper on Web, hence the Node(handle) rewraps.
 */
...
class Player(godotObject: GodotHandle) :
  ...
  private fun actionShoot() {
    ...
    repeat(weapon.shotCount.toInt()) {
      ...
      impact.play("shot")
      // SceneTree.get_root has no Web family: decals ride the Player's scene parent instead.
