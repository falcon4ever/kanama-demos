...
import net.multigesture.kanama.api.ThreadedLoad
...
import net.multigesture.kanama.api.buttonGroup
import net.multigesture.kanama.api.buttonPressed
import net.multigesture.kanama.api.callDeferred
...
import net.multigesture.kanama.api.grabFocus
import net.multigesture.kanama.api.isClass
import net.multigesture.kanama.api.loadThreadedGetPackedScene
import net.multigesture.kanama.api.loadThreadedGetStatusWithProgress
import net.multigesture.kanama.api.loadThreadedRequest
import net.multigesture.kanama.api.pushError
import net.multigesture.kanama.api.setProcess
import net.multigesture.kanama.api.text
...
class Menu(godotObject: GodotHandle) : KanamaScript<Node>(godotObject, ::Node) {
  ...
  private val metalFxSupported = false
  ...
  fun ready() {
    ...
    // Web builds are single-player: no lobby signal can fire (see the multiplayer facade).
    ...
    listOf(
      ...
      .forEach { makeButtonGroup(self.getNodeOrNull("UI/Settings/$it")?.let { node -> Node(node.handle) }) }
  }
  ...
  fun process(delta: Double) {
    ...
    when (val status = load.status) {
      ThreadedLoad.THREAD_LOAD_IN_PROGRESS -> {
      ...
      ThreadedLoad.THREAD_LOAD_LOADED -> {
      ...
      ThreadedLoad.THREAD_LOAD_FAILED,
      ThreadedLoad.THREAD_LOAD_INVALID_RESOURCE -> {
  ...
  private fun makeButtonGroup(commonParent: Node?) {
    ...
    TpsFactory.buttonGroup().use { group: ButtonGroup ->
  ...
  private fun approximately(value: Double, expected: Double): Boolean =
    Mathf.abs(value - expected) < 0.0001
  ...
  fun onLoadingDoneTimerTimeout() {
    ...
    val scene =
      ResourceLoader.loadThreadedGetPackedScene(TpsScenes.LEVEL)
  ...
  fun onSettingsPressed() {
    ...
    setPressed(
      "DisplayMode/Windowed",
      displayMode == Window.MODE_WINDOWED || displayMode == Window.MODE_MAXIMIZED,
    )
    ...
    setOneLong(
      "VSync",
      listOf(
        "Disabled" to DisplayServer.VSYNC_DISABLED,
        "Enabled" to DisplayServer.VSYNC_ENABLED,
        "Adaptive" to DisplayServer.VSYNC_ADAPTIVE,
        "Mailbox" to DisplayServer.VSYNC_MAILBOX,
      ),
      TpsSettings.videoLong("vsync"),
    )
    setOneLong(
      "MaxFPS",
      listOf(
    ...
    val scaleFilters =
      listOf(
        "Bilinear" to Viewport.SCALING_3D_MODE_BILINEAR,
        "FSR1" to Viewport.SCALING_3D_MODE_FSR,
        "MetalFXSpatial" to Viewport.SCALING_3D_MODE_METALFX_SPATIAL,
        "FSR2" to Viewport.SCALING_3D_MODE_FSR2,
        "MetalFXTemporal" to Viewport.SCALING_3D_MODE_METALFX_TEMPORAL,
      )
    ...
    setOneLong(
      "GIType",
      listOf(
        "LightmapGI" to TpsSettings.LIGHTMAP_GI,
        "VoxelGI" to TpsSettings.VOXEL_GI,
        "SDFGI" to TpsSettings.SDFGI,
      ),
      TpsSettings.renderLong("gi_type"),
    )
    setOneLong(
      "GIQuality",
      listOf(
        "Disabled" to TpsSettings.GI_DISABLED,
        "Low" to TpsSettings.GI_LOW,
        "High" to TpsSettings.GI_HIGH,
      ),
      TpsSettings.renderLong("gi_quality"),
    )
    setOneBool("TAA", TpsSettings.renderBool("taa"))
    setOneLong(
      "MSAA",
      listOf(
        "Disabled" to Viewport.MSAA_DISABLED,
        "2X" to Viewport.MSAA_2X,
        "4X" to Viewport.MSAA_4X,
        "8X" to Viewport.MSAA_8X,
      ),
      TpsSettings.renderLong("msaa"),
    )
    ...
    setOneLong(
      ...
      listOf(
        "Disabled" to -1L,
        "Medium" to RenderingServerQuality.ENV_SSAO_QUALITY_MEDIUM,
        "High" to RenderingServerQuality.ENV_SSAO_QUALITY_HIGH,
      ),
      TpsSettings.renderLong("ssao_quality"),
    )
    setOneLong(
      "SSIL",
      listOf(
        "Disabled" to -1L,
        "Medium" to RenderingServerQuality.ENV_SSIL_QUALITY_MEDIUM,
        "High" to RenderingServerQuality.ENV_SSIL_QUALITY_HIGH,
      ),
      TpsSettings.renderLong("ssil_quality"),
    )
  ...
  fun onApplyPressed() {
    ...
    when {
      pressed("DisplayMode/Windowed") ->
        config.setValue("video", "display_mode", Window.MODE_WINDOWED)
      pressed("DisplayMode/Fullscreen") ->
        config.setValue("video", "display_mode", Window.MODE_FULLSCREEN)
      pressed("DisplayMode/ExclusiveFullscreen") ->
        config.setValue("video", "display_mode", Window.MODE_EXCLUSIVE_FULLSCREEN)
    }
    firstPressedValue(
        "VSync",
        listOf(
      ...
      ?.let { config.setValue("video", "vsync", it) }
    firstPressedValue(
        "MaxFPS",
        listOf(
          "30" to 30L,
      ...
      ?.let { config.setValue("video", "max_fps", it) }
    firstPressedValue(
        "ResolutionScale",
        listOf(
          "UltraPerformance" to 1.0 / 3.0,
          "Performance" to 1.0 / 2.0,
          "Balanced" to 1.0 / 1.7,
          "Quality" to 1.0 / 1.5,
          "UltraQuality" to 1.0 / 1.3,
          "Native" to 1.0,
        ),
      )
      ?.let { config.setValue("video", "resolution_scale", it) }
    firstPressedValue(
        "ScaleFilter",
        listOf(
          "Bilinear" to Viewport.SCALING_3D_MODE_BILINEAR,
      ...
      ?.let { config.setValue("video", "scale_filter", it) }
    firstPressedValue(
        "GIType",
        listOf(
          "LightmapGI" to TpsSettings.LIGHTMAP_GI,
      ...
      ?.let { config.setValue("rendering", "gi_type", it) }
    firstPressedValue(
        "GIQuality",
        listOf(
          "Disabled" to TpsSettings.GI_DISABLED,
      ...
      ?.let { config.setValue("rendering", "gi_quality", it) }
    ...
    firstPressedValue(
      ...
      ?.let { config.setValue("rendering", "msaa", it) }
    ...
    firstPressedValue(
      ...
      ?.let { config.setValue("rendering", "ssao_quality", it) }
    firstPressedValue(
        "SSIL",
        listOf(
          "Disabled" to -1L,
      ...
      ?.let { config.setValue("rendering", "ssil_quality", it) }
  ...
  fun onCancelPressed() {
    if (online.isVisible()) resetOnlinePeer()
    main.show()
    playButton.grabFocus()
    settingsMenu.hide()
    online.hide()
  ...
  fun onHostPressed() {
    ...
    onlineStatus.text = "Hosting on ${preferredLanAddress() ?: "this device"}:$port."
  ...
  @Signal("replace_main_scene") fun replaceMainScene(scene: PackedScene) = Unit
}
