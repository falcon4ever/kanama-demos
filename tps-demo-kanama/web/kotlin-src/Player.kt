...
import net.multigesture.kanama.api.getTimeLeft
...
class Player(godotObject: GodotHandle) :
  ...
  fun animate(anim: Long, delta: Double = 0.0) {
    ...
    when (anim) {
      ...
      AnimationState.JUMP_DOWN.id ->
        animationTree.set("parameters/state/transition_request", "jump_down")
  ...
  private fun applyInput(delta: Double) {
    ...
    val cameraZ = cameraBasis.z.withY(0.0).normalized()
    val cameraX = cameraBasis.x.withY(0.0).normalized()
    ...
    if (onAir) {
      animate(
        if (self.velocity.y > 0.0) AnimationState.JUMP_UP.id else AnimationState.JUMP_DOWN.id,
        delta,
      )
      ...
      if (target.length() > 0.001) {
        ...
        orientation =
          orientation.withBasis(Basis(qFrom.slerp(qTo, delta * ROTATION_INTERPOLATE_SPEED)))
      }
  ...
  private fun shootBullet() {
    ...
    self.getParent()?.let { Node(it.handle).addChild(bulletNode, true) }
