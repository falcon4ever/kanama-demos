...
This folder is a Kanama/Kotlin port of Godot's Third Person Shooter demo.
The original project is maintained at
https://github.com/godotengine/tps-demo.

The port keeps the original Godot scenes and assets, while gameplay scripts are
ported to Kotlin under `kotlin-src/`. The source snapshot and import notes are
recorded in [UPSTREAM.md](UPSTREAM.md).

Original third person shooter demo made using
...
You need [Godot Engine](https://godotengine.org), JDK 25+, and a sibling Kanama
checkout to run this port from source.

From the `kanama-demos` repository root:

```sh
./gradlew tpsBuildAndRunGodot
```

From this folder directly:

```sh
./gradlew buildAndRunGodot
```
