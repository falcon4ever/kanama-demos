...
import net.multigesture.kanama.api.getOverlappingBodies
...
class Grenade(godotObject: GodotHandle) :
  ...
  fun ready() {
    ...
    explosionStartTimer.signal(Timer.Signals.timeout).connect(self, argumentCount = 0) {
      explode()
    }
  }
  ...
  private fun explode() {
    ...
    for (body in explosionArea.getOverlappingBodies()) {
      val body3d = Node3D(body.handle)
      if (Node(body.handle).isInGroup("damageables") && !body3d.isPlayer()) {
        var impactPoint = (self.globalPosition - body3d.globalPosition).normalized()
    ...
    if (explosion != null) {
      self.getParent()?.let { parent -> Node(parent.handle).addChild(explosion) }
      Node3D(explosion.handle).globalPosition = self.globalPosition
    }

    self.hide()
    kanamaScope.launch {
      explosionSound.signal(AudioStreamPlayer3D.Signals.finished).await(self, argumentCount = 0)
      self.queueFree()
    }
  }
