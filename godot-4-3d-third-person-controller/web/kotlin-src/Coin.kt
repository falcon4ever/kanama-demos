...
import net.multigesture.kanama.api.setCollisionLayerValue
import net.multigesture.kanama.types.Vector3
import kotlinx.coroutines.launch

@ScriptClass(attachTo = "RigidBody3D")
class Coin(godotObject: GodotHandle) :
  ...
  fun spawn(coinDelay: Double = 0.5) {
    ...
    playerDetectionArea.signal(Area3D.Signals.bodyEntered).connectObject(self) { body ->
  ...
  fun follow(offset: Double) {
    ...
    self.globalPosition = initialTweenPosition.lerp(Node3D(currentTarget.handle).globalPosition, offset)
  }
  ...
  fun collect() {
    ...
    kanamaScope.launch {
      collectAudio.signal(AudioStreamPlayer3D.Signals.finished).await(self, argumentCount = 0)
      self.queueFree()
    }
  }
