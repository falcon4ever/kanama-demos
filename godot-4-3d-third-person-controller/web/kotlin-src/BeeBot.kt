...
class BeeBot(godotObject: GodotHandle) :
  KanamaScript<RigidBody3D>(godotObject, ::RigidBody3D), KanamaCoroutineOwner {
  override val kanamaScope = KanamaScope()

  @ScriptProperty var shootTimer: Double = 1.5

  @ScriptProperty var bulletSpeed: Double = 6.0

  @ScriptProperty var coinsCount: Long = 5
  ...
  fun ready() {
    ...
    beeRoot =
      self.requireAs("MeshRoot/bee_root", ::Node3D).kotlinScriptInstance<BeeRoot>()
        ?: error("MeshRoot/bee_root is missing BeeRoot script instance")
    defeatSound = self.requireAs("DefeatSound", ::AudioStreamPlayer3D)

    bodyEnteredConnection =
      detectionArea.signal(Area3D.Signals.bodyEntered).connectObject(self) { body ->
        onBodyEntered(Node3D(body.handle))
      }
    bodyExitedConnection =
      detectionArea.signal(Area3D.Signals.bodyExited).connectObject(self) { body ->
        onBodyExited(Node3D(body.handle))
      }
  ...
  fun physicsProcess(delta: Double) {
    ...
    if (currentTarget != null && alive) {
      if (self.globalPosition.distanceSquaredTo(currentTarget.globalPosition) > LOOK_AT_EPSILON) {
        // The bee is unparented at the level root: local == global, so the interpolated
        // look uses the global transform pair (scale-1 rig).
        val targetTransform = self.globalTransform.lookingAt(currentTarget.globalPosition)
        self.globalTransform = self.globalTransform.interpolateWith(targetTransform, 0.1)
      }
  ...
  fun damage(impactPoint: Vector3, force: Vector3) {
    ...
    kanamaScope.launch {
      ...
      if (puff != null) {
        self.getParent()?.let { parent -> Node(parent.handle).addChild(puff) }
