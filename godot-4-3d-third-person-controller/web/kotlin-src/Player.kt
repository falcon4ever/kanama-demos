...
/**
 * Web port of the third-person player. Adaptations: input actions come from project.godot (the
 * runtime InputMap registration is desktop-only), and transforms ride the pure-Kotlin
 * basis/quaternion math over the mirrored snapshots.
 */
...
class Player(godotObject: GodotHandle) :
  ...
  @ScriptProperty var bulletSpeed = 10.0

  @ScriptProperty var attackImpulse = 10.0

  @ScriptProperty var acceleration = 4.0

  @ScriptProperty var jumpInitialImpulse = 12.0

  @ScriptProperty var jumpAdditionalForce = 4.5

  @ScriptProperty var rotationSpeed = 12.0

  @ScriptProperty var stoppingSpeed = 1.0

  @ScriptProperty var maxThrowbackForce = 15.0

  @ScriptProperty var shootCooldown = 0.5

  @ScriptProperty var grenadeCooldown = 0.5
  ...
  private lateinit var uiAimReticle: Control
  ...
  fun ready() {
    ...
    cameraController =
      cameraControllerNode.kotlinScriptInstance<CameraController>()
        ?: error("CameraController is missing CameraController script instance")
    attackAnimationPlayer =
      self.requireAs("CharacterRotationRoot/MeleeAnchor/AnimationPlayer", ::AnimationPlayer)
    groundShapeCast = self.requireAs("GroundShapeCast", ::ShapeCast3D)
    grenadeAimControllerNode = self.requireAs("GrenadeLauncher", ::Node3D)
    grenadeAimController =
      grenadeAimControllerNode.kotlinScriptInstance<GrenadeLauncher>()
    ...
    characterSkin =
      characterSkinNode.kotlinScriptInstance<CharacterSkin>()
        ?: error("CharacterSkin is missing CharacterSkin script instance")
    uiAimReticle = self.requireAs("%AimReticle", ::Control)
    uiCoinsContainer =
      self.requireAs("%CoinsContainer", ::Node).kotlinScriptInstance<CoinsContainer>()
    ...
    cameraController.setupAnchor(self, this)
    grenadeAimControllerNode.visible = false
    emitWeaponSwitched()

    DemoScenes.warmUpBulletPool(self)

    characterSkinNode.signal(CharacterSkinNames.Signals.stepped).connect(
      self,
      argumentCount = 0,
    ) {
      playFootStepSound()
  ...
  fun physicsProcess(delta: Double) {
    ...
    if (Input.isActionJustPressed("swap_weapons")) {
      equippedWeapon =
        if (equippedWeapon == WeaponType.GRENADE) WeaponType.DEFAULT else WeaponType.GRENADE
      grenadeAimControllerNode.visible = equippedWeapon == WeaponType.GRENADE
      emitWeaponSwitched()
    }
    ...
    if (isAiming) {
      lastStrongDirection =
        (cameraControllerNode.globalTransform.basis * Vector3.BACK).normalized()
    }
  ...
  fun shoot() {
    ...
    DemoScenes.launchBullet(
      self.getParent()?.let { Node(it.handle) },
      self,
      origin,
  ...
  fun loseCoins() {
    ...
    repeat(lostCoins.toInt()) {
      ...
      val coin =
        coinNode.kotlinScriptInstance<Coin>()
          ?: error("Coin scene is missing Coin script instance")
      self.getParent()?.let { parent -> Node(parent.handle).addChild(coinNode) }
  ...
  @Signal("weapon_switched") fun weaponSwitched(weaponName: String) = Unit
  ...
  private fun orientCharacterToDirection(direction: Vector3, delta: Double) {
    ...
    rotationRoot.basis =
      Basis(currentRotation.slerp(rotationBasis, delta * rotationSpeed)).scaled(modelScale)
  }
