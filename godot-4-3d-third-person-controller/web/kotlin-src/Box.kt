...
class Box(godotObject: GodotHandle) :
  ...
  fun damage(impactPoint: Vector3, force: Vector3) {
    ...
    if (destroyedBox != null) {
      self.getParent()?.let { parent -> Node(parent.handle).addChild(destroyedBox) }
    ...
    kanamaScope.launch {
      destroySound.signal(AudioStreamPlayer3D.Signals.finished).await(self, argumentCount = 0)
      self.queueFree()
    }
  }
