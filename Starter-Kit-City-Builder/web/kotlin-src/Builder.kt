...
import net.multigesture.kanama.api.ScriptResource
import net.multigesture.kanama.api.asObject
...
import net.multigesture.kanama.api.getMousePosition
import net.multigesture.kanama.api.getState
import net.multigesture.kanama.api.isClass
import net.multigesture.kanama.api.kotlinScriptInstance
import net.multigesture.kanama.api.load
...
class Builder(godotObject: GodotHandle) : KanamaScript<Node3D>(godotObject, ::Node3D) {
  ...
  // loaded from the resource cache, which owns it). Released on reassign / exit_tree. Web
  // adaptation: the owning wrapper is the ScriptResource handle wrapper.
  private var mapOwner: ScriptResource? = null
  ...
  fun process(delta: Double) {
    ...
    val gridmapPosition =
      Vector3i(
        worldPosition.x.let { Mathf.roundToInt(it).toInt() },
        0,
        worldPosition.z.let { Mathf.roundToInt(it).toInt() },
      )
  ...
  private fun loadMap(path: String) {
    ...
    map =
      ResourceLoader.load(path)?.asObject()?.kotlinScriptInstance<DataMap>()?.let {
        adoptLoadedMap(it)
      } ?: adoptFreshMap()
