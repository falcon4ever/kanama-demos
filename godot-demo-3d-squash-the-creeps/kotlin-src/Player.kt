...
class Player(godotObject: GodotHandle) :
  ...
  fun physicsProcess(delta: Double) {
    ...
    if (Input.isActionPressed("move_right")) direction = direction.withX(direction.x + 1.0)
    if (Input.isActionPressed("move_left")) direction = direction.withX(direction.x - 1.0)
    if (Input.isActionPressed("move_back")) direction = direction.withZ(direction.z + 1.0)
    if (Input.isActionPressed("move_forward")) direction = direction.withZ(direction.z - 1.0)

    if (direction != Vector3.ZERO) {
      direction = direction.normalized()
      // Upstream sets basis = Basis.looking_at(direction); the same orientation (at the
      // unchanged position) comes from look_at_from_position toward the move direction.
      // Setting the orientation will affect the rotation of the node.
      self.lookAtFromPosition(self.position, self.position + direction, Vector3.UP)
    ...
    velocity = velocity.withX(direction.x * speed.toDouble()).withZ(direction.z * speed.toDouble())

    // Jumping.
    if (self.isOnFloor() && Input.isActionJustPressed("jump")) {
      velocity = velocity.withY(velocity.y + jumpImpulse.toDouble())
    }
    ...
    velocity = velocity.withY(velocity.y - fallAcceleration.toDouble() * delta)
    ...
    for (index in 0 until self.getSlideCollisionCount()) {
      var bounced = false
      val collision = self.getSlideCollision(index)
      if (collision != null) {
        try {
          ...
          if (collider != null) {
            ...
            if (colliderNode.isInGroup("mob") && Vector3.UP.dot(collision.getNormal()) > 0.1) {
              collider.kotlinScriptInstance<Mob>()?.squash()
              self.velocity = self.velocity.withY(bounceImpulse.toDouble())
              bounced = true
            }
          }
        } finally {
    ...
    self.rotation = rotation.withX(Mathf.PI / 6.0 * self.velocity.y / jumpImpulse.toDouble())
  }
